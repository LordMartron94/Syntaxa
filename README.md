...
- **Parser**: `SyntaxaParser` is built from a grammar package (with entry rule set). `SyntaxaParserCreate(grammarPackage, ..., getAnalysis)` takes a `GrammarPackage` and an optional `getAnalysis` (e.g. `lowering.GetAnalysis(grammarPackage)`) for nullable/first/follow when using Predict or Pratt; pass nil otherwise. `SyntaxaParserParseWithContext(parser, ctx)` runs the parse. For LST allocation tuning, `SetNodePoolPrefill(n)` pre-allocates reusable node buffers per context, and `SetNodePoolGrowFn(func(minNeeded int) int)` customizes growth when the pool runs empty.
