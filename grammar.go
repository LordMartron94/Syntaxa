...
/*
Lookahead describes one lookahead condition: at the given offset (in the visible token stream),
the token must equal Expected. Used by Predict-style rules to record disambiguation conditions
on the grammar for introspection and tooling.
*/
type Lookahead[TToken comparable] struct {
	Offset   int
	Expected TToken
}
...
type Grammar[TToken, TNodeKind comparable] struct {
	...
	*/
	...
	/*
		Lookaheads, when non-nil, are the lookahead conditions under which this production is selected
		(e.g. in a Predict wrapper). Each entry specifies Offset and Expected token. Set by the rule
		factory when using PredictWithLookahead or PredictLookahead.
	*/
	Lookaheads []Lookahead[TToken]
}
